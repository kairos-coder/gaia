// titans/iapetos/remember.js
const IapetosMemory = {
  layers: { fast: null, local: null, communal: null },
  init: async function() { try { const r = await fetch('titans/iapetos/index.json'); this.layers.fast = await r.json(); } catch(e) { this.layers.fast = { endings: [], stats: {} }; } this.layers.local = this._loadLocalMemory(); this._connectGaiaDB().then(d => { this.layers.communal = d; }).catch(() => {}); },
  ...
  _loadLocalMemory: function() { const s = localStorage.getItem('iapetos_memory'); return s ? (() => { try { return JSON.parse(s); } catch(e) {} })() : { events: [], last_tick: 0, total_events: 0 }; },
  _saveLocalMemory: function() { localStorage.setItem('iapetos_memory', JSON.stringify(this.layers.local)); },
  _queryFast: function(e) { return (this.layers.fast?.endings || []).filter(en => en.type === e.type).slice(-10); },
  ...
  _updateFastRecall: function(e) { if (!this.layers.fast) return; this.layers.fast.stats.total_endings++; this.layers.fast.endings.push(e); },
...
if (typeof module !== 'undefined') module.exports = IapetosMemory;
